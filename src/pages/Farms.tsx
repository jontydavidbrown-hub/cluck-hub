...
export default function Farms() {
  // include setFarms so we can sync the context immediately on delete
  const { farms = [], farmId, setFarmId, setFarms, createFarm } = useFarm() as any;
  ...
  // Prefer global array (even if empty) to avoid flicker/fallback to stale context
  const farmsForUI: any[] = Array.isArray(globalFarms) ? globalFarms : (farms || []);
  ...
  }, [farmId, globalSelectedFarmId, setGlobalSelectedFarmId, setFarmId]);
  ...
  async function onCreateFarm() {
    ...
    if (!name) return;
    await createFarm?.(name); // provider updates storage; no extra nudge (prevents duplicates)
    setNewName("");
  }
  ...
  function onDeleteFarm(id: string) {
    ...
    // Update BOTH global and context immediately so the deletion sticks visually
    setGlobalFarms(prev => {
    ...
    setFarms((prev: any[]) => (prev || []).filter(f => f.id !== id));
  }
